...
{
	int value, option, number, counter = 0, amount, hold = 0;
	...
	if(value == 1)
	{
		...
		printf("Minimum Qty: %d\n", item[counter].minQuantity);
		printf("   Is Taxed: %s\n", taxed);
		printf("Please enter the quantity %s; Maximum of %d or 0 to abort: ", stock, amount);
		scanf("%d", &option);
		if(option == 0)
		{
			printf("--== Aborted! ==--\n");
		}
		if(option > item[counter].quantity)
		{
			hold = getIntLimited(0, item[counter].quantity);
		}
		else
		{
			if(item[counter].quantity > item[counter].minQuantity)
			{
				item[counter].quantity = item[counter].quantity - hold;
				printf("--== Checked out! ==--\n");
			}
			else
			{
...
void GrocInvSys (){
    ...
    if(record == 1)
	{
        ...
        /* if(SEL == 6)
		{
			...
			//b = deleteItem(inventory, holdNR);
            pause();
        }
        if(SEL == 7)
		{
            printf("Not implemented!\n");
			//b = searchByName(inventory, holdNR);
            pause();
        } */
        if(SEL == 0)
		{
